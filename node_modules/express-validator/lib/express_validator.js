...
var expressValidator = function expressValidator(options) {
  options = options || {};
  var defaults = {
    customValidators: {},
    customSanitizers: {},
    errorFormatter: function errorFormatter(param, msg, value, location) {
      return {
        location: location,
        param: param,
        msg: msg,
  ...
  function Sanitizer(param, req, locations) {
    ...
    req[sanitizerSymbol].forEach(function (customSanitizers) {
  ...
  function createValidationChain(field, location, message, req, contexts) {
    ...
    chain.notEmpty = function () {
      return chain.isLength({ min: 1 });
    };
    chain.len = chain.isLength;

    req[validatorChainSymbol].forEach(function (customValidators) {
      Object.keys(customValidators).forEach(function (name) {
        chain[name] = function () {
    ...
    req[sanitizerSymbol].forEach(function (customSanitizers) {
      Object.keys(customSanitizers).forEach(function (name) {
        chain[name] = function () {
  ...
  function validateSchema(schema, req, loc, contexts) {
    ...
    for (var param in schema) {
      ...
      for (var methodName in paramCfg) {
        ...
        var opts = methodCfg.options || [];
        if (opts != null && !Array.isArray(opts)) {
          opts = [opts];
        }

        validator[methodName].apply(validator, _toConsumableArray(opts));
  ...
  return function (req, res, next) {
    ...
    function runContexts() {
      contexts.filter(function (context) {
        ...
        var field = selectFields(req, context)[0];
        if (!field) {
          context.promise = Promise.resolve();
          return;
        }

        var promises = context.validators.map(function (validatorCfg) {
    ...
    req.asyncValidationErrors = function (mapped) {
      runContexts();
      return Promise.all(req._asyncValidationErrors).then(function () {
